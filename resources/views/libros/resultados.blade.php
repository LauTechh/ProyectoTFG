            ...
            <div class="zona-busqueda-resultados" style="margin-bottom: 30px;">
                <form action="{{ route('libros.buscar') }}" method="GET" style="display: flex; gap: 10px;">
                    <div style="flex: 1; position: relative;">
                        <input type="text" name="query" value="{{ request('query') }}" placeholder="Buscar otro libro..."
                    ...
                    <button type="submit"
                        style="background: #fb923c; color: white; border: none; padding: 10px 25px; border-radius: 20px; font-weight: bold; cursor: pointer; transition: 0.3s;">
                        🔍
                    </button>
                </form>
            </div>

            <h1 class="titulo-invitado">🥔 Resultados para: <span style="font-weight: 400;">"{{ request('query') }}"</span>
            </h1>
            ...
            <div class="lista-resultados">
                @forelse($libros as $libro)
                    ...
                    <div class="fila-libro">
                        ...
                        <div class="acciones-libro">
                            @auth
                                <button class="btn-compacto-add btn-añadir-libro" onclick="añadirLibroSinRecargar(this)"
                                    data-title="{{ $info['title'] ?? '' }}"
                                    data-author="{{ implode(', ', $info['authors'] ?? []) }}"
                                    data-genre="{{ $libro['genero_nuestro'] ?? 'Ficción' }}" data-cover="{{ $portada }}">
                                    + Añadir
                                </button>
                            @endauth
                        </div>
                ...
                @empty
                    <div style="text-align: center; padding: 40px; background: white; border-radius: 20px;">
                        <p style="color: #8b5e3c; font-size: 1.1rem;">No hay patatas... digo, libros para
                            "{{ request('query') }}". 🥔</p>
                    </div>
                @endforelse
            </div>
